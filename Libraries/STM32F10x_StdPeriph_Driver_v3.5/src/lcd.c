...
static void LCD_WR_REG(uint16_t LCD_Reg)
{
	...
	// LCD_RS(0) Low, send command 
	GPIO_ResetBits(GPIOD, GPIO_Pin_13);

	// LCD_CS(0) Falling Edge, use LCD Chip
	GPIO_ResetBits(GPIOC, GPIO_Pin_6);
	
	// LCD_WR(0)PD14 Falling Edge, wrtie data on display
...
static uint16_t LCD_ReadReg(uint16_t LCD_Reg)
{
	...
	// Chip Select: Falling edge -> lcd chip use
